...
#if defined(NDEBUG)
# error "Gtaccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf41fa8657921d1cfe900958d608e44ebaf8741c31ef9c0a4f77e1d577bcad04c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gtaccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gtaccoin Signed Message:\n";
...
{
    // Gtaccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   if (nHeight ==1)
    {
     int64 nSubsidy = 5000000000 *COIN;
...
static const int64 nTargetTimespan = 1 *24 * 60 * 60; //3.5 * 24 * 60 * 60; // Gtaccoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60;//2.5 * 60; // Gtaccoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gtaccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gtaccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf41fa8657921d1cfe900958d608e44ebaf8741c31ef9c0a4f77e1d577bcad04c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Gtaccoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x79abd329f0b3926a6a89c6be714d301383a91ca2234f5928f4dcb3bb5d97a44a"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Gtaccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GtaccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GtaccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GtaccoinMiner(CWallet *pwallet)
{
    printf("GtaccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gtaccoin-miner");
    ...
    try { loop {
        ...
        printf("Running GtaccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GtaccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GtaccoinMiner, pwallet));
}
